...
c.execute("DROP TABLE IF EXISTS mean")
c.execute("DROP TABLE IF EXISTS shap_p_values_text")

c.execute("Create TABLE main_data (Id int, dice_value int)")
c.execute("Create TABLE mean (Id int, mean float)")
...
while True:
    ...
    c.execute("INSERT INTO mean values ({},{})".format(i + 1, b_mean))
    connection.commit()
    i += 1
    time.sleep(0.5)